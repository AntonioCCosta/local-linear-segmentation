...
cpdef loglik_mvn(np.ndarray[np.float64_t, ndim=2] theta, np.ndarray[np.float64_t, ndim=2] y,int lag=1):
    ...
    #in principle there is not need to this because if sigma is well-conditioned then the determinant must be
    #positive (the covariance is a positive semi-definite matrix)
...
cpdef R_null(np.ndarray[np.float64_t, ndim=2] theta,np.ndarray[np.float64_t, ndim=2] window1,np.ndarray[np.float64_t, ndim=2] window2,int N,int lag):
    ...
    for i in range(cN):
        ...
        loglik1=loglik_mvn(theta_g1,obs,clag)
        loglik2=loglik_mvn(theta_g2,obs,clag)
        r=loglik2-loglik1
        Rs[i] = r
    return Rs
